...
全文精读，并视觉核对 Fig.1 的能级纲图、Fig.2 的 angular-distribution 双解与联合 `P-R_ac` 约束、Table 1 的 polarization/`R_ac`/mixing ratio，以及 Fig.4 的实验-QTR 比较。

本页按 `experiment-ingest + project-ingest` 建立反对方证据链。它不把 Lv 2022 的作者结论写成本 Wiki 的最终裁决。
...
本页建立 counter-source，不裁决争议。用户已完成 source 页面及 L22-1 至 L22-13 的核对，并校正 L22-13 对 Fig.3(d)、p.5 正文与 Fig.4(c) 的证据边界。
