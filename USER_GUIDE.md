...
`knowledge/research-notes/` 保存少量高价值、尚未成熟的“暂定研究推理”。普通问答默认不把它当作正式证据，也不会自动检索或创建；只有你明确要求研究启发、假设、迁移、reflect/project/synthesis 时才会使用，并会标明 provisional 状态、回到原始 source 核查。创建 note 不等于人工审核或知识晋升。
