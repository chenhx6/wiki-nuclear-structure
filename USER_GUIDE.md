...
- Wiki 不能替代原文阅读；
- 未完成人工复核的页面或 claim 只能作为阅读线索，不能直接作为论文依据；
...
如果 Codex 判断剩余上下文、token、5 小时额度或执行余量不足以稳定完成任务，会进入 safe suspend：停止扩大修改范围，检查 Git 状态和 diff，把已完成/未完成事项、修改文件、风险及可直接使用的续跑提示词写入 handoff，然后请你在额度刷新后发送“继续”。这不会让原任务自动睡眠或原地恢复；后续 automation 也应视为一次读取 handoff 的新任务。除非你明确要求，safe suspend 不会创建 automation，也不会 commit/push。
