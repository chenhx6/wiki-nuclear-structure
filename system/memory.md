...
- L3 边界只作用于受信任的 `E:\imp\wiki`：项目级 `.codex/config.toml` 选择 `wiki_l3 + approval_policy=never`，profile 保留整个 Wiki（包括 `.git/`、`.codex/`、`.agents/` 与按任务授权的 raw 路径）的 write，Wiki 外只读并仅执行可证明零外部写入的程序；raw 和任务授权边界仍独立生效。Desktop 可在运行时对 `.codex/`、`.agents/` 施加宿主保护 DENY，但不因此把 profile 改成 read，也不清理 ACL。周测不得修改项目配置或仓库 Skill；根目录/`.git` 真实写探针和两个保护哨兵的可读性才是 automation 权限事实来源，保护目录 DENY 只作诊断。Codex 不得请求外部写入权限；外部状态变更只能由用户在 Codex 外手动完成，再由新会话核验。机器级 requirements 不得锁定其他项目；其他项目使用 `:workspace + on-request`。Wiki 会话允许公网、局域网、localhost 与代理，行为上禁用 Computer Use；Browser/Chrome 只做检索、验证和显式 Wiki 路径下载。权限边界验收完成后，L3 科学自治仍须由用户另行发起。
