...
- Nature Skills 已按用户要求完成全量更新并在重启 Codex 后生效：stable clone 位于 `%USERPROFILE%\ai-skills\nature-skills`，同步目标为 `%USERPROFILE%\.codex\skills`。2026-08-13 clone 与全局安装均更新至 `d1fb103c635e63288c0f2174460276b1ab7582d8`，安装 19 个顶层 skills；`nature-shared` 已替代旧 `_shared`，并保留 `nature-paper-card`。Windows 本机本次采用 PowerShell 原生 `git pull --ff-only`、受控 `robocopy` 镜像和逐文件 SHA-256 核验；源目录与目标目录内容一致。严格 `robocopy /MIR /L` 检查报告的两个差异仅是目标端运行时 `__pycache__` 缓存目录，不是 skill 内容缺失。后续 Windows 更新优先沿用 PowerShell 路径，不要直接用 WSL `bash` 调用 CRLF checkout 的 `.sh`，以免再次触发 `set -o pipefail` 解析错误。这些写作、阅读、图件、引用、返修等外部 skills 是科研写作辅助层，不能覆盖 Wiki evidence policy、paper evidence gate、source/raw locator 复核和用户科学判断。
