...
如果提示目录“正由另一进程使用”，通常是 Codex 或 Nature Skills 的 MCP/Python/uv 服务仍在打开旧技能。请先关闭所有 Codex 窗口，等待几秒让相关 MCP 服务退出；如果任务管理器中仍有明确属于 Nature Skills 的 `uv`/`python` 进程，再结束这些相关进程，然后重新双击更新脚本。更新器不会自动终止进程，也不会强制删除被占用目录。
