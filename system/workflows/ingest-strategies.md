...
Staged evidence reading 是阅读顺序优化，不是降低摄入标准。默认正式摄入不应停留在 `metadata-only` 或 `skimmed`；普通正式摄入至少达到 `read`。核心争议文献、关键实验文献、重要理论框架文献和 project/synthesis 主证据链文献应以 `deep-read` 级核查为目标。
...
## Human review triage 控长

P0/P1 是复盘重点，不把所有 `needs_review` 等权重铺开。

- `P0 focus: top 3-5`：详细列文件、section/claim ID、source locator、为什么重要、用户检查什么、不审核的风险。
- `Remaining P0`：所有剩余 P0 必须可见，但用压缩格式列文件、section/claim ID、locator 状态和风险关键词。
- `P1`：最多逐项列 5 个最重要位置，其余按文件聚合。
- `P2/P3`：只聚合，不逐条展开；低风险 index/overview/handoff/log 与科学 claim 分开。
- 没有 P0 时写 `P0: none identified`。
- 始终给出“精力有限时建议先看”的 3-5 个位置；不足 3 个时列全部实际审核点。
...
3. Human review triage：按 P0 focus、Remaining P0、P1、P2/P3 输出。
