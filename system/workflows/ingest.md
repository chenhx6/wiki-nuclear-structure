...
5. 不 push；
6. 最终复盘列出 WIP hash、message、未 push、待审核文件和待审核 claim ID。
...
4. 确认 HEAD 是对应的 WIP ingest commit 后，使用 `git commit --amend` 把 WIP 转换为 final commit，不新建额外 review commit；
5. final commit message 由 Codex 根据实际摄入与审核修改推荐生成，用户无需手写；
6. 检查通过后，只有用户允许 final push 时才执行 `git push origin main`；
7. final push 后确认工作树中没有残留该文献摄入相关 diff。
