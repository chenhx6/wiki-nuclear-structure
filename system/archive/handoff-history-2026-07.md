...
- Windows 已安装 QMD `2.5.3`；Quarto `1.9.38` 保留为独立的可选发布工具，两者不混用。
- 仓库根目录已初始化 project-local `.qmd/index.sqlite` 并由 `.gitignore` 排除；collection `nuclear-knowledge` 仅覆盖 `knowledge/**/*.md`，不索引 raw、system、outputs 或 Obsidian 状态。
- 当前索引为 118 个 Markdown 文件、348 个向量 chunk；embedding freshness 和抽样向量复现通过。
- `qmd pull` 已下载/校验 embedding、generation 和 reranking 模型；它是模型管理命令。禁止使用 `qmd update --pull` 让 QMD 隐式操作 Git。
- BM25、语义和完整 hybrid 查询均真实通过；本机冷启动实测约为十秒级、两分钟和七分钟，因此默认使用 direct read/`rg`/BM25，语义查询按需，完整 hybrid 仅用于高价值跨页综合并允许降级。
- QMD 搜索结果只作候选导航；事实、数值与物理边界必须回读完整知识页、source 和必要的 raw locator。
- 已同步 AGENTS、check、query/lint workflow、USER_GUIDE、详细指南、PLAN、memory、handoff 和 log；未修改任何科学知识页或 raw 文件。
- `qmd doctor` 核心检查通过；其将下载器生成的 67 字节 `.etag` 元数据误报为无效 GGUF，但三类真实查询和 embedding vector sample 均通过。本轮不为消除该非阻断警告重复下载约 2.2 GB 模型。
- 用户原有 `.obsidian/graph.json` 与 `raw/zotero/wiki-inbox.bib` 修改继续保持独立，未被覆盖。
...
- 按 `theory-ingest + project-ingest` 全文精读 11 页，并视觉核对 Figs.1-13 与 Tables I-II；新建 `knowledge/sources/nomura-2022-questioning-wobbling-ibfm.md`，建立 NOM22-1 至 NOM22-15，并在本轮用户审核后全部改为 `needs_review: false`，source 升级为 `human-reviewed`。
...
- Project 更新：umbrella 新增 LSW-IBFM-1 至 LSW-IBFM-4；gamma-soft evidence map 新增 GSD-PROJ-7；这 5 条 project notes 已全部由用户审核并改为 `needs_review: false`。`135Pr` project 只加理论 source role；`133La/127Xe/105Pd` 只留 future-case map；`187Au` 只记录本文明确的模型适用困难。
- 按用户批注修正两处关键表述：NOM22-10 现在明确 `105Pd` 原 wobbling paper 中的 band B/C 对应本文的 B1/B2；NOM22-11 改写为分别陈述 `B(E2)out/B(E2)in` 与 `B(M1)out/B(E2)in`，避免把 Fig.9/Table II 的两类相对量混写。为保持 `133La` 记述一致，NOM22-8 也同步改为显式写出两个比值。
...
- 当前 lint：114 pages、1124 Wikilinks、20/20 source hash、0 error、10 warning、0 info；source claims 223 条且 `claim_needs_review=0`。另有 5 条 project notes 由 overview 人工记录。
- 当前待审队列已清零；本轮收尾为最终 commit/push，仅保留用户未暂存的 `.obsidian/graph.json` 与 `raw/zotero/wiki-inbox.bib`。
- 文档同步门：无目录、命令、治理规则、schema、template 或 vocabulary 变化，不更新 USER_GUIDE/AGENTS/check；不修改用户拥有的 PLAN。
- 本轮按用户要求收尾为 final commit/push；既有本地 WIP 为 `WIP ingest: Nomura 2022 IBFM low-spin wobbling`。用户 `.obsidian/graph.json` 与 `raw/zotero/wiki-inbox.bib` 修改保持未暂存、绝不纳入提交。
...
- 已建立 Human review triage protocol：文献摄入、project、synthesis、data-analysis-bridge 与 claim-review-update 按 P0/P1/P2/P3 输出审核重点，P0 全列并给出风险，P1 限前 10，P2/P3 聚合，同时提供精力有限时优先看的 3–5 个位置。
...
- 未发表数据的隔离与加密策略尚未确定。
- QMD 已启用；后续观察真实查询的召回质量、冷启动时间与磁盘成本，再决定是否接入 MCP 常驻服务。
